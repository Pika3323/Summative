...
void Dankey::Tick(float delta, std::vector<Character*> *Curr){
	...
	if (this->BarrelDelay == 40) {
		if (this->direction == ECharacterDirection::R_Left){
			Curr->push_back(new Barrel(direction, Vector2D(this->position.x, this->position.y + 48)));
			this->BarrelDelay = 0;
		}
		else {
			Curr->push_back(new Barrel(direction, Vector2D(this->position.x + 64, this->position.y + 48)));
			this->BarrelDelay = 0;
		}
	}
}
